...
try:
    from ..utils.ocr_processor import PDFOCRProcessor, OCRTextBlock
    OCR_AVAILABLE = True
except ImportError:
    OCR_AVAILABLE = False
    # Define dummy classes for type hints when OCR is not available
    class OCRTextBlock:
        pass
    logging.warning("OCR processor not available - scanned document processing limited")
...
class PyMuPDFParser(BasePDFParser):
    ...
    def __init__(self, pdf_path: str):
        ...
        if OCR_AVAILABLE:
            try:
                ...
                self._logger.info("OCR processor initialized for scanned document support")
            except Exception as e:
                self._logger.warning(f"Failed to initialize OCR processor: {str(e)}")
                self._ocr_processor = None
        else:
            self._logger.info("OCR capabilities not available - scanned documents will use fallback text extraction")
    ...
    def extract_text_with_positions(self) -> List[TextBlock]:
        """
        Extract text elements with detailed positioning and formatting information.
        
        This method performs comprehensive text extraction optimized for heading
        detection algorithms. Each text block includes precise positioning,
        font information, and formatting details required for rule-based
        heading classification.
        
        Text extraction process:
        1. Iterate through all document pages
        2. Extract text blocks with bounding boxes
        3. Capture font names, sizes, and style flags
        4. Preserve Unicode characters for multilingual support
        5. Calculate relative positioning for layout analysis
        6. Filter out non-text elements and artifacts
        
        Font analysis includes:
        - Font family and specific font names
        - Font sizes for heading size analysis
        - Style flags (bold, italic, underline)
        - Color information where available
        
        Position analysis includes:
        - Absolute coordinates (x, y) on page
        - Text block dimensions (width, height)
        - Relative positioning for indentation detection
        - Line spacing and paragraph boundaries
        
        Returns:
            List of TextBlock objects containing:
            - Unicode-safe text content
            - Precise positioning coordinates
            - Complete font and style information
            - Page references for cross-page analysis
            - Layout context for heading detection
    ...
    def _convert_ocr_blocks_to_text_blocks(self, ocr_blocks: List[OCRTextBlock]) -> List[TextBlock]:
        """
